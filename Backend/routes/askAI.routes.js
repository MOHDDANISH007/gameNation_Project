...
router.post("/post", authenticateUser, async (req, res) => {
  try {
    ...
    const [ps4Games, ps5Games, xboxXGames, xboxOneGames] = await Promise.all([
      ps4GamesSchema.find({}, { base_name: 1, game_id: 1, platform: 1, _id: 0 }).lean(),
      ps5GamesSchema.find({}, { base_name: 1, game_id: 1, platform: 1, _id: 0 }).lean(),
      xboxXGamesSchema.find({}, { base_name: 1, game_id: 1, platform: 1, _id: 0 }).lean(),
      xboxoneGamesSchema.find({}, { base_name: 1, game_id: 1, platform: 1, _id: 0 }).lean()
    ]);
